...
// Lazy accessor — window.api is injected by the preload script at runtime.
// Capturing it at module load time breaks on Windows dev because the preload
// may not have attached yet when the module is first imported.
...
export async function stopTimer(teamId) {
  return api().request({ method: 'POST', path: `/team/${teamId}/time_entries/stop` })
}
